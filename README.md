...
- **魔法链接免密登录**：采用 OTP Magic Link 模式，无需繁琐的密码，输入邮箱即可一键完成安全注册/登录。且已完美克服邮箱防病毒组件提前”扫链接“导致一次性令牌失效的问题，在客户端予以了友好的交互提醒与指引。
...
- `app/login/page.tsx`: 基于 Supabase Magic Link 实现的安全无密码登录门户。
